...
interface Question { left: number; right: number; answer: number }
...
interface Invader { id: number; num: number; alive: boolean }
...
function makeInvaders(answer: number): Invader[] {
  ...
  for (let i = 0; i < nums.length; i++) {
    if (nums[i] === 0) {
      let n: number
      do { n = Math.floor(Math.random() * 9) + 1 } while (n === answer)
      nums[i] = n
    }
  }
...
export function SpaceMathScreen({ onBack }: Props) {
  ...
  const [question, setQuestion]   = useState<Question>(() => questionRef.current)
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (keys.left)  cannonXRef.current = Math.max(24, cannonXRef.current - CANNON_SPD)
      if (keys.right) cannonXRef.current = Math.min(arenaW - 24, cannonXRef.current + CANNON_SPD)
      ...
      if (blt) {
        ...
        for (let row = 0; row < ROWS && !hit; row++) {
          for (let col = 0; col < COLS && !hit; col++) {
            ...
            if (
              blt.x >= ix && blt.x <= ix + INV_W &&
              blt.y >= iy && blt.y <= iy + INV_H
            ) {
              hit = true
              bulletRef.current = null
              setBullet(null)
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      ...
      if (e.key === ' ')          { e.preventDefault(); keysRef.current.fire  = true }
    }
  ...
  const arenaW = arenaRef.current?.clientWidth ?? 360
  const initialX = (arenaW - gridW) / 2
    ...
    <GameShell onBack={onBack} percent={(score / WIN_SCORE) * 100} score={score} className="spmath">
      ...
      <div className="spmath-lives">
        {Array.from({ length: LIVES_START }, (_, i) => (
          <span key={i} className={`spmath-heart ${i < lives ? '' : 'spmath-heart--lost'}`}>❤️</span>
        ))}
      </div>

      {/* Question */}
      <div className={`spmath-question ${flash === 'correct' ? 'spmath-question--correct' : flash === 'wrong' ? 'spmath-question--wrong' : ''}`}>
        <div className="spmath-eq">
          {Array.from({ length: question.left }, (_, i) => (
            <span key={i} className="spmath-emoji">{EMOJIS_ADD[question.left % EMOJIS_ADD.length]}</span>
          ))}
          <span className="spmath-op">+</span>
          {Array.from({ length: question.right }, (_, i) => (
            <span key={i} className="spmath-emoji">{EMOJIS_ADD[question.right % EMOJIS_ADD.length]}</span>
          ))}
          <span className="spmath-op">=</span>
      ...
      <div className="spmath-arena" ref={arenaRef}>
        ...
        {invState.map((inv, idx) => {
          ...
          const col = idx % COLS
          const x = initialX + invXRef.current + col * (INV_W + INV_GAP_X)
          const y = 60 + invYRef.current + row * (INV_H + INV_GAP_Y)
          return (
        ...
        {/* Bullet */}
        {bullet && (
          <div className="spmath-bullet" style={{ left: bullet.x - 4, top: bullet.y }} />
        )}

        {/* Cannon */}
        <div className="spmath-cannon" style={{ left: cannonX - 24 }}>🚀</div>
        ...
        {phase !== 'playing' && (
          <div className="spmath-overlay">
            <div className="spmath-overlay-card">
              ...
              <div className="spmath-overlay-title">{phase === 'win' ? 'You win!' : 'Game over!'}</div>
