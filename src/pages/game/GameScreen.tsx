...
import { GameTopbar } from '@common/components/GameTopbar/GameTopbar'
import { StreakBadge } from '@common/components/StreakBadge/StreakBadge'
import type {
  AnimalItem,
  BuildNumberItem,
  BuildWordItem,
...
import { ClockBlanksDisplay } from './components/ClockBlanksDisplay/ClockBlanksDisplay'
import { ColorDisplay } from './components/ColorDisplay/ColorDisplay'
import { CountingDisplay } from './components/CountingDisplay/CountingDisplay'
import { HearPressDisplay } from './components/HearPressDisplay/HearPressDisplay'
...
import { MathMostDisplay } from './components/MathMostDisplay/MathMostDisplay'
import { NumberBlanksDisplay } from './components/NumberBlanksDisplay/NumberBlanksDisplay'
import { TimerBar } from './components/TimerBar/TimerBar'
import { VirtualKeyboard } from './components/VirtualKeyboard/VirtualKeyboard'
import { WhatNextDisplay } from './components/WhatNextDisplay/WhatNextDisplay'
import { WhichMoreDisplay } from './components/WhichMoreDisplay/WhichMoreDisplay'
...
function buildSequence(game: GameConfig, isRandom: boolean): GameItem[] {
  ...
  if (game.type === 'buildNumber') {
    return game.generateItems(isRandom)
  }
  if (game.type === 'whichMore') {
    return game.generateItems(isRandom)
  }
  if (game.type === 'mathAdd') {
    return game.generateItems(isRandom)
  }
  if (game.type === 'mathSub') {
    return game.generateItems(isRandom)
  }
  if (game.type === 'mathMost') {
    return game.generateItems(isRandom)
  }
  if (game.type === 'whatNext') {
    return game.generateItems(isRandom)
  }
  if (game.type === 'whatBefore') {
    return game.generateItems(isRandom)
  }
  if (game.type === 'animalSounds') {
    return isRandom ? shuffle(game.items) : [...game.items]
  }
  if (game.type === 'buildWord') {
    return isRandom ? shuffle(game.items) : [...game.items]
  }
  if (game.type === 'colorMatch') {
    return isRandom ? shuffle(game.items) : [...game.items]
  }
  if (game.type === 'clock') {
    return game.generateItems(isRandom)
...
function getExpectedKey(item: GameItem, gameType: string | undefined): string {
  ...
  if (gameType === 'animalSounds') {
    return (item as AnimalItem).key
  }
  if (gameType === 'whichMore') {
    return (item as WhichMoreItem).answer
  }
  if (gameType === 'mathAdd' || gameType === 'mathSub') {
    return (item as MathItem).answer
  }
  if (gameType === 'mathMost') {
    return (item as MathMostItem).answer
  }
  if (gameType === 'whatNext' || gameType === 'whatBefore') {
    return (item as WhatNextItem).answer.toUpperCase()
  }
  if (gameType === 'colorMatch') {
    return (item as ColorItem).key
  }
...
function getHintSpeech(
  ...
  if (gameType === 'numberWords') {
    return (item as NumberWordItem).word
  }
  if (gameType === 'animalSounds') {
    return (item as AnimalItem).name
  }
  if (gameType === 'buildNumber') {
    return (item as BuildNumberItem).word
  }
  if (gameType === 'whichMore') {
  ...
  if (gameType === 'colorMatch') {
    return `${(item as ColorItem).name}! Press the first letter`
  }
  if (gameType === 'buildWord') {
    return (item as BuildWordItem).word
  }
  if (gameType === 'clock') {
    return (item as ClockItem).speech
  }
...
export function GameScreen({ game, isRandom, onBack, onComplete }: GameScreenProps) {
  ...
  const expectLower =
    game.type === undefined ? (game as { expectLower?: boolean }).expectLower : false
    ...
    (key: string) => {
      ...
      if (isBuildNumber) {
        if (pressed >= '0' && pressed <= '9') {
          ...
          if (pressed === expectedDigit) {
            ...
            if (newBuffer.length === item.digits.length) {
              handleCorrect()
            }
          } else {
            handleWrong()
      ...
      if (isBuildWord) {
        ...
        if (pressed === expectedLetter) {
          const newBuffer = buildBuffer + pressed
          setBuildBuffer(newBuffer)
          if (newBuffer.length === item.letters.length) {
            handleCorrect()
          }
        } else {
          handleWrong()
  ...
  const getProgressText = () => {
    ...
    if (isNumberWords || isWhichMore || isMath || isMathMost) {
    ...
    if (isClock) {
      return `Clock ${currentIndex + 1} of ${sequence.length}`
    }
    if (isAnimalSounds) {
      return `Animal ${currentIndex + 1} of ${sequence.length}`
    }
    if (isColorMatch) {
      return `Color ${currentIndex + 1} of ${sequence.length}`
    }
    if (isWhatNext) {
      return `${currentIndex + 1} of ${sequence.length}`
    }
  ...
  const getPromptText = () => {
    ...
    if (isAnimalSounds) {
      return 'Press the first letter!'
    }
    if (isColorMatch) {
      return 'Press the first letter!'
    }
    if (isWhichMore) {
      return 'Press the bigger number!'
    }
    if (isMath) {
      return 'Type the answer!'
    }
    if (isMathMost) {
      return 'Press the biggest group!'
    }
    if (isWhatNext) {
      return 'What comes next?'
    }
    if (isWhatBefore) {
      return 'What comes before?'
    }
    if (audioOnly) {
      return 'Press what you hear!'
    }
    if (expectLower) {
      return 'Press the lowercase letter!'
    }
  ...
  const renderDisplay = () => {
    ...
    if (isAnimalSounds) {
      return (
        <AnimalDisplay item={currentItem as AnimalItem} feedback={feedback} animKey={animKey} />
      )
    }
    if (isBuildNumber) {
    ...
    if (isColorMatch) {
      return <ColorDisplay item={currentItem as ColorItem} feedback={feedback} animKey={animKey} />
    }
    if (isClock) {
      return (
        <ClockBlanksDisplay
          item={currentItem as ClockItem}
          filled={buildBuffer}
          feedback={feedback}
          shakeKey={shakeKey}
        />
    ...
    if (isWhichMore) {
      return (
        <WhichMoreDisplay
          item={currentItem as WhichMoreItem}
          feedback={feedback}
    ...
    if (isMath) {
      return <MathDisplay item={currentItem as MathItem} feedback={feedback} animKey={animKey} />
    }
    if (isMathMost) {
      return (
        <MathMostDisplay
          item={currentItem as MathMostItem}
          feedback={feedback}
          pressedKey={lastPressed}
          animKey={animKey}
        />
    ...
    if (isWhatNext) {
      return (
        <WhatNextDisplay item={currentItem as WhatNextItem} feedback={feedback} animKey={animKey} />
      )
    }
    if (isWhatBefore) {
      return (
        <WhatNextDisplay
          item={currentItem as WhatNextItem}
          feedback={feedback}
          animKey={animKey}
          reversed
        />
    ...
    if (audioOnly) {
      return <HearPressDisplay feedback={feedback} animKey={animKey} />
    }
